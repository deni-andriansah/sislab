...
class PRuanganController extends Controller
{
    ...
    {
        $p_ruangan = p_ruangan::find($id);

        if (!$p_ruangan) {
            return redirect()->route('p_ruangan.index')->with('error', 'Data pengembalian tidak ditemukan.');
        }

        // Ambil data peminjaman ruangan terkait
        $pm_ruangan = pm_ruangan::find($p_ruangan->id_pm_ruangan);

        if (!$pm_ruangan) {
            return redirect()->route('p_ruangan.index')->with('error', 'Data peminjaman tidak ditemukan.');
        }

        // Ambil semua ruangan yang dikembalikan dan ubah statusnya kembali ke "Dipinjam"
        $detailRuangan = peminjamandetailruangan::where('id_pm_ruangan', $pm_ruangan->id)->get();
